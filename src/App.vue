...
async function promptService() {
    ...
    const response = await axios.post('http://localhost:3000/api/answer', {
        prompt: prompt.value
    }).then(res => res.data).catch(err => {
        console.error(err)
        return 'Erro ao se comunicar com o servidor'
            ...
            <div class="flex flex-col space-y-4">
                <div class="flex flex-col space-y-2">
                    <div class="flex flex-col items-stretch">
                        ...
                        <textarea v-model="prompt" @keyup.enter="promptBoost" @click="toggleDropdown" id="prompt"
                            type="text" :disabled="isWriting"
                            class="w-full p-2 border border-gray-300 rounded-md min-h-[100px] max-h-[200px]" />
                        <Listbox v-model="selected" v-if="showDropdown" as="div" class="mt-1 relative">
                            ...
                            <ListboxOptions
                                ...
                                <ListboxOption class="text-gray-900 cursor-default select-none relative py-2 pl-10 pr-4">
                                    <button class="font-normal block truncate w-full text-left" @click="promptComplete">Complete o Texto</button>
                                    <span class="text-green-600 absolute inset-y-0 left-0 flex items-center pl-3">
                                        <ChevronDoubleUpIcon class="w-5 h-5" aria-hidden="true" />
                                    </span>
                                </ListboxOption>
                                <ListboxOption class="text-gray-900 cursor-default select-none relative py-2 pl-10 pr-4">
                                    <button class="font-normal block truncate w-full text-left" @click="promptSimplify">Simplifique o Texto</button>
                                    <span class="text-green-600 absolute inset-y-0 left-0 flex items-center pl-3">
                                        <ChevronDoubleUpIcon class="w-5 h-5" aria-hidden="true" />
                                    </span>
                                </ListboxOption>
                                ...
                                <ListboxOption class="text-gray-900 cursor-default select-none relative py-2 pl-10 pr-4">
                                    <button class="font-normal block truncate w-full text-left"
                                        @click="promptService">Responder um problema</button>
                                    <span class="text-green-600 absolute inset-y-0 left-0 flex items-center pl-3">
                                        <ChevronDoubleUpIcon class="w-5 h-5" aria-hidden="true" />
                                    </span>
                                </ListboxOption>
